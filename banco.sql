...
create unique index if not exists alunos_nome_uq
on public.alunos (nome);

alter table public.alunos
add column if not exists turma text default '2A';
...
create index if not exists idx_mensalidades_ano_mes
on public.mensalidades (ano, mes);
...
begin
  if not exists(
    ...
    and policyname='pub_read_alunos'
  ) then
    create policy "pub_read_alunos"
    on public.alunos
    for select
    using (true);
  end if;
  ...
  if not exists(
    select 1 from pg_policies
    where tablename='alunos'
    and policyname='auth_all_alunos'
  ) then
    create policy "auth_all_alunos"
    on public.alunos
    for all
...
begin
  if not exists(
    ...
    and policyname='pub_read_movs'
  ) then
    create policy "pub_read_movs"
  ...
  if not exists(
    select 1 from pg_policies
    where tablename='movimentacoes'
    and policyname='auth_all_movs'
  ) then
    create policy "auth_all_movs"
    on public.movimentacoes
    for all
    using (auth.role()='authenticated')
    with check (auth.role()='authenticated');
...
begin
  if not exists(
    select 1 from pg_policies
    where tablename='mensalidades_config'
    and policyname='pub_read_cfg'
  ) then
    create policy "pub_read_cfg"
  ...
  if not exists(
    select 1 from pg_policies
    where tablename='mensalidades_config'
    and policyname='auth_all_cfg'
  ) then
    create policy "auth_all_cfg"
    on public.mensalidades_config
    for all
    using (auth.role()='authenticated')
    with check (auth.role()='authenticated');
...
begin
  if not exists(
    select 1 from pg_policies
    where tablename='mensalidades'
    and policyname='pub_read_mens'
  ) then
    create policy "pub_read_mens"
  ...
  if not exists(
    select 1 from pg_policies
    where tablename='mensalidades'
    and policyname='pub_update_mens'
  ) then
    create policy "pub_update_mens"
    on public.mensalidades
    for update
    using (true)
    with check (true);
  ...
  if not exists(
    select 1 from pg_policies
    where tablename='mensalidades'
    and policyname='auth_all_mens'
  ) then
    create policy "auth_all_mens"
    on public.mensalidades
    for all
    using (auth.role()='authenticated')
    with check (auth.role()='authenticated');
...
begin
  if not exists(
    ...
    and policyname='pub_upload_comprovantes'
  ) then
    create policy "pub_upload_comprovantes"
    on storage.objects
    for insert
  ...
  if not exists(
    ...
    where tablename='objects'
    and schemaname='storage'
    and policyname='pub_read_comprovantes'
  ) then
    create policy "pub_read_comprovantes"
    on storage.objects
    for select
  ...
  if not exists(
    ...
    where tablename='objects'
    and schemaname='storage'
    and policyname='auth_del_comprovantes'
  ) then
    create policy "auth_del_comprovantes"
    on storage.objects
    for delete
    using (
...
insert into public.mensalidades_config (
  valor_mensal,
  dia_vencimento
)
select 15.00, 10
where not exists (
  select 1
  from public.mensalidades_config
  limit 1
);
